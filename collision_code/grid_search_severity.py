...
Usage:
  python grid_search_severity_csv.py \
    --data /path/to/US_Accidents.csv \
    --max-rows 500000 \
    --target Severity \
    --out-dir runs/
"""
...
def prepare_xy(df: pd.DataFrame, target: str, max_cat_card: int = 100):
    df = df[df[target].notna()]
    df = df[df[target].isin([1, 2, 3, 4])]

    for c in ["City", "County", "Weather_Condition"]:
        clip_high_cardinality(df, c, top_k=max_cat_card)
